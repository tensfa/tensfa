...
trainX = np.array(np.random.random((100, 50, 1)), dtype=np.float32)
trainY = np.array(np.random.random(100), dtype=np.float32)

# create and fit the LSTM network
model = Sequential()
model.add(LSTM(32, input_shape=trainX.shape[1:], activation='tanh',recurrent_activation='sigmoid',return_sequences=True))
