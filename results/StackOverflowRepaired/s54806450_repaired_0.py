...
final_training_set = np.array(np.random.random((n, time_steps, n_features)),dtype=np.float32)
labels = np.random.randint(0, n_classes, n, dtype=np.int32)
...
x= tf.placeholder('float32',[None,time_steps,n_features])
...
###########################################
logit=tf.matmul(outputs[-1],out_weights)+out_bias
